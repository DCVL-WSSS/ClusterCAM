...
def do_python_eval(
    ...
    def compare(start, step, TP, P, T, input_type, threshold):
        for idx in range(start, len(name_list), step):
            ...
            if input_type == "png":
                ...
                if bg:
                    tensor = np.zeros((num_cls + 1, h, w), np.float32)
    ...
    for i in range(8):
        p = multiprocessing.Process(
            target=compare, args=(i, 8, TP, P, T, input_type, threshold)
        )
        p.start()
        p_list.append(p)
    for p in p_list:
    ...
    for i in range(num_cls):
        ...
        FP_ALL.append(
            (P[i].value - TP[i].value) / (T[i].value + P[i].value - TP[i].value + 1e-10)
        )
        FN_ALL.append(
            (T[i].value - TP[i].value) / (T[i].value + P[i].value - TP[i].value + 1e-10)
        )
    loglist = {}
    for i in range(num_cls):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--list", default="./VOC2012/ImageSets/Segmentation/train.txt", type=str
    )
    ...
    if args.type == "npy":
        assert args.t is not None or args.curve or args.bg
    ...
    if not args.curve:
        ...
        for i in range(args.start, args.end):
            t = i / 100.0
            loglist = do_python_eval(
                args.predict_dir, args.gt_dir, name_list, args.num_classes, args.type, t
            )
            l.append(loglist["mIoU"])
            print(
                "%d/60 background score: %.3f\tmIoU: %.3f%%" % (i, t, loglist["mIoU"])
            )
        ...
        writelog(
            args.logfile,
            {"mIoU": l, "Best mIoU": max_mIoU, "Best threshold": best_thr},
            args.comment,
        )
