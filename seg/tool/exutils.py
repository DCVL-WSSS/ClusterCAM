...
def RandomCrop(imgarr, cropsize):
    ...
    img_container[cont_top : cont_top + ch, cont_left : cont_left + cw] = imgarr[
...
def get_data_from_chunk(chunk, args):
    ...
    for i, pieces in enumerate(chunk):
        ...
        gt_temp = np.asarray(
            Image.open(os.path.join(args.seg_pgt_path, piece + ".png"))
        )
        ...
        ori_temp[:, :, 0] = (img_temp[:, :, 0] * 0.229 + 0.485) * 255.0
        ori_temp[:, :, 1] = (img_temp[:, :, 1] * 0.224 + 0.456) * 255.0
        ori_temp[:, :, 2] = (img_temp[:, :, 2] * 0.225 + 0.406) * 255.0
