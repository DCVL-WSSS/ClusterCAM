...
if __name__ == "__main__":
    ...
    criterion = torch.nn.CrossEntropyLoss(
        weight=None, ignore_index=255, reduction="elementwise_mean"
    ).cuda()
    ...
    for ep in range(args.num_epochs):
        for iter in range(num_batches_per_epoch):
            ...
            images, ori_images, seg_labels, img_names = exutils.get_data_from_chunk(
                chunk, args
            )
            ...
            pred = model(x=images)
            pred = F.interpolate(
                pred, size=(w, h), mode="bilinear", align_corners=False
            )
        ...
        torch.save(
            ...
            os.path.join(save_path, args.session_name + str(ep) + ".pth"),
        )
